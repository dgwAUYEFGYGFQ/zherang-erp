(function (window) {
  ...
  ChintPrototypeShell.registerPageComponent(componentName, {
    ...
    data() {
      ...
      return {
        ...
        lines: [],
        history: [{ version: source.sapVersion, rate: source.sapDiscountRate, status: 'SAP同步成功', time: source.lastSyncTime, operator: '陈静怡' }],
    ...
    computed: {
      selectedSnapshot() {
        if (this.activeView === 'sap') return { version: this.form.sapVersion, rate: this.form.sapDiscountRate, status: 'SAP生效版本', time: this.form.lastSyncTime, operator: '陈静怡' };
        if (this.activeView === 'history') return this.history.find((item) => item.version === this.historyVersion) || this.history[0];
        return { version: this.form.currentVersion, rate: this.form.discountRate, status: this.syncLabel(this.form.syncStatus), time: this.form.updateTime, operator: '陈静怡' };
      },
      viewRate() { return Number(this.selectedSnapshot ? this.selectedSnapshot.rate : 0); },
      viewLines() {
        const rate = this.viewRate / 100;
        return this.lines.map((item) => ({ ...item, discountAmount: round(item.amount * rate), afterAmount: round(item.amount - item.amount * rate) }));
      },
      originalAmount() { return round(this.lines.reduce((sum, item) => sum + item.amount, 0)); },
      discountAmount() { return round(this.originalAmount * this.viewRate / 100); },
      afterAmount() { return round(this.originalAmount - this.discountAmount); },
      ...
      hasUnsaved() { return this.receiptRefreshPending || Math.abs(Number(this.form.discountRate) - Number(this.persisted.discountRate)) > 0.0001; },
      differsFromSap() { return Math.abs(Number(this.form.discountRate) - Number(this.form.sapDiscountRate)) > 0.0001 || this.form.currentVersion !== this.form.sapVersion; },
    },
    methods: {
      makeLines() {
        const mock = ChintPrototypeShell.getMockData('rebateSettlements', {}) || {};
        ...
        }).map((line) => ({ ...line, m2UntaxedPrice: Number(line.m2UntaxedPrice != null ? line.m2UntaxedPrice : (line.m2UnitPrice != null ? line.m2UnitPrice : line.unitPrice || 0)) }));
      },
      ...
      submitSap() {
        ...
        ElementPlus.ElMessageBox.confirm('确认将' + this.form.currentVersion + '重新提交SAP？', '重新提交SAP', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
          ...
          window.setTimeout(() => {
            ...
            this.form.sapVersion = this.form.currentVersion; this.form.sapDiscountRate = this.form.discountRate;
            ...
            this.history.unshift({ version: this.form.sapVersion, rate: this.form.sapDiscountRate, status: 'SAP同步成功', time: this.form.lastSyncTime, operator: '陈静怡' });
      ...
      <div class="flow-progress-layout">
        ...
        <section class="panel flow-panel-shell" data-tour="settlement-detail-snapshot">
          ...
          <div class="panel-body">
            ...
            <el-form v-if="activeView === 'current'" :model="form" label-width="96px" size="small" style="margin-top:14px"><div class="form-grid" style="grid-template-columns:repeat(3,minmax(0,1fr));gap:0 18px"><el-form-item label="折让比例"><el-input-number v-model="form.discountRate" :min="0" :max="100" :precision="2" :step="0.5" style="width:100%"></el-input-number></el-form-item><el-form-item label="SAP比例"><el-input :model-value="rate(form.sapDiscountRate)" disabled></el-input></el-form-item><el-form-item label="占用对账单"><el-input :model-value="form.reconciliationId || '未占用'" disabled></el-input></el-form-item></div></el-form>
            <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:12px"><div style="padding:11px;background:var(--el-fill-color-light);border-radius:6px"><small>原金额</small><div style="font-size:17px;font-weight:650">{{ money(originalAmount) }}</div></div><div style="padding:11px;background:var(--el-color-warning-light-9);border-radius:6px"><small>折让金额 · {{ rate(viewRate) }}</small><div style="font-size:17px;font-weight:650">{{ money(discountAmount) }}</div></div><div style="padding:11px;background:var(--el-color-success-light-9);border-radius:6px"><small>折后金额</small><div style="font-size:17px;font-weight:650">{{ money(afterAmount) }}</div></div></div>
          </div>
        </section>
  ...
  ChintPrototypeShell.registerRoute({
    ...
    guideSteps: [
      ...
      { target: '[data-tour="settlement-detail-lines"]', title: '核对独立收货明细', description: '明细按结算年月、公司、供应商和玻璃品类独立取得，再按所选版本比例计算，负数保留原符号。' },
